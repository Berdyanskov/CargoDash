...
function emitNodeCtor(id, data, varName, schemas, fnNamesByNode, voteCtorByJudgeId, modelVarByNodeId) {
    switch (data.kind) {
        ...
        case "DataOutput": {
            ...
            if (d.preserveOrder) {
                throw new CodegenError(`DataOutput "${varName}": preserve_order=true is not yet implemented ` +
...
function emitModelSpec(data, varName) {
    switch (data.modelKind) {
        case "remote": {
            ...
            if (data.timeout !== 60)
                args.push(`timeout=${data.timeout}`);
            if (data.maxRetries !== 5)
                args.push(`max_retries=${data.maxRetries}`);
            if (data.backoffBase !== 1)
                args.push(`backoff_base=${data.backoffBase}`);
            if (data.backoffMax !== 30)
                args.push(`backoff_max=${data.backoffMax}`);
            if (data.jitter !== 0.5)
                args.push(`jitter=${data.jitter}`);
