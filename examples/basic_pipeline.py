...
from cargodash import (
    Batch, Schema,
    RawDataSource, DataOutput, Processor, MapProcessor, Judge, Vote,
    Pipeline,
)
...
def augment_row(row: dict) -> dict:
    return {**row, "text": row["text"] + " [AUG]"}


def log_drop(batch: Batch):
    for row in batch.rows:
        print(f"  [dropped] id={row['id']}  quality={row['quality']}")
    # Returning nothing -> no downstream output (Processor handles None).
...
def main() -> None:
    ...
    clean = MapProcessor(clean_row, input_schema=schema, output_schema=schema)
    ...
    augment = MapProcessor(augment_row, input_schema=schema, output_schema=schema,
